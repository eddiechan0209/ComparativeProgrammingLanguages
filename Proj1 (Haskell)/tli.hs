...
lookupVar name [] = 0
lookupVar name ((id,v):rest) = if (x == name) then v else lookupVar name rest
								where x = if (isLabel id) then init id else id
...
run listTuple lineNum env output = let (env1, _, output1, lineNum1) = perform (findStmt listTuple lineNum) env lineNum [] output 
									in (if (round lineNum1 > length listTuple) then output1 else run listTuple lineNum1 env1 output1)    
...
updateST ((first:rest), lineNum) env =
    if (isLabel first) then (first, lineNum):env else env --init drops the colon
...
--    contents <- hGetContents pfile
--    putStr (run (map oldParseLine (map words (lines contents))) [] "")
--    hClose pfile